...
export function ListeningHeatmap({ data, isLoading }: ListeningHeatmapProps) {
  const maxIntensity = useMemo(
    () => Math.max(1, ...data.map((p) => p.intensity)),
    [data],
  );
  ...
  if (isLoading) {
    return (
      <div className="h-full w-full overflow-x-auto pb-4">
        <div className="min-w-[450px] grid grid-cols-[auto_repeat(24,1fr)] gap-[2px]">
  ...
  return (
    <div className="h-full bg-card/30 border border-border/50 rounded-xl p-3 w-full overflow-x-auto">
      <div className="min-w-[450px] flex flex-col gap-1">
        <div className="grid grid-cols-[32px_repeat(24,1fr)] gap-[2px]">
          <div />
          {Array.from({ length: 24 }).map((_, h) => (
            <div
              key={h}
        ...
        {DAYS.map((day, dayIndex) => (
          <div
            key={day}
            className="grid grid-cols-[32px_repeat(24,1fr)] gap-[2px] items-center"
          >
